...
**Platform**: React + TypeScript + Vite PWA (Progressive Web App)
- Works on any modern browser (desktop, phone, tablet) — installable to homescreen as native-like app
...
- Schema covers: profiles, sessions, exercise results, curriculum progress, targets, statistics, journal entries, template library

**Accessibility**: Screen-reader compatible, keyboard navigation, voice-only mode for visually impaired users
- All visual elements have semantic HTML + ARIA labels
...
- Must work completely offline after initial load (binaural beats sourced externally)
